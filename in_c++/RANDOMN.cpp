...
// RandomNoCooperationGateway implementation
RandomNoCooperationGateway::RandomNoCooperationGateway(const vector<FogNode>& fog_nodes, const vector<CloudService>& cloud_services)
...
vector<Task> RandomNoCooperationGateway::get_next_batch(const vector<Task>& all_tasks) {
    // Algorithm 4: Random task selection implementation
    current_batch++;
    
    // Skip already processed tasks
    vector<Task> unprocessed_tasks;
    for (auto& task : all_tasks) {
        if (find(processed_tasks.begin(), processed_tasks.end(), task.id) == processed_tasks.end()) {
            unprocessed_tasks.push_back(task);
        }
    }
    
    if (unprocessed_tasks.empty()) {
        return {};
    }
    
    // Random selection of tasks using random sampling
    vector<Task> random_tasks;
    
    if (unprocessed_tasks.size() <= batch_size) {
        random_tasks = unprocessed_tasks;
    } else {
        // Create a vector of indices for all unprocessed tasks
        vector<int> indices(unprocessed_tasks.size());
        ...
        for (int i = 0; i < batch_size && i < indices.size(); i++) {
            random_tasks.push_back(unprocessed_tasks[indices[i]]);
        }
    }
...
int RandomNoCooperationGateway::offload_task(Task& task) {
    ...
    if (task.should_go_to_fog()) {
        ...
        // Find best fog node based on location
        ...
        // No cooperation in this algorithm, so if the best node is not available,
        // the task goes directly to cloud
    }
    
    // If no suitable fog node or not a fog candidate, process on cloud
    process_cloud(task);
...
void RandomNoCooperationGateway::print_fog_status() {
...
/*
int main() {
    cout << "=== Random No Cooperation Algorithm ===" << endl;
    ...
    // Create gateway
    RandomNoCooperationGateway gateway(fog_nodes, cloud_services);
    ...
    try {
        ...
        cout << "RandomNoCooperation: Total = " << fixed << setprecision(2) << avg_processing_time 
             << ", Fog = " << avg_fog_time << ", Cloud = " << avg_cloud_time << endl;
        
        // Power consumption
        cout << "\n=== Power Consumption (W) ===" << endl;
        cout << "RandomNoCooperation: [";
        ...
        cout << "RandomNoCooperation: " << fixed << setprecision(2) << avg_queue_delay << endl;
        ...
        cout << "RandomNoCooperation: Fog = " << fog_count << " (" << fog_percent << "%), Cloud = " 
        ...
        double avg_alt_node_selection = 0.0; // Not applicable for non-cooperation algorithm
        ...
        cout << "RandomNoCooperation: Node = " << fixed << setprecision(4) << avg_node_selection 
