...
# Import all four algorithm implementations
# Each file contains the same class name but with different implementations
...
def run_policy(policy_module, policy_name, tasks):
    """Run a specific policy and return results"""
    # Initialize fog nodes for this run
    initialize_fog_nodes()
    
    # Create a simple cloud service object
    cloud_services = [{
        "name": "Cloud-DC-01",
        "location": (0, 0),
        "ram": 100000,  # 100 GB
    ...
    gateway = policy_module.FCFSCooperationGateway(FOG_NODES, cloud_services)
    
    # Process all tasks with progress bar
    with tqdm(total=len(tasks), desc=f"Processing {policy_name}") as pbar:
        for task in tasks:
            # Create a fresh copy of the task for this run
            task_copy = Task(task.id, task.data_type, task.size, task.arrival_time)
            gateway.offload_task(task_copy)
            pbar.update(1)
...
def analyze_results(results):
    ...
    for policy_name, gateway in results.items():
        ...
        print(f"{policy_name}: Total = {avg_total:.2f}, Fog = {avg_fog:.2f}, Cloud = {avg_cloud:.2f}")
    
    # Power consumption
    print("\n=== Average Power Consumption per Node (W) ===")
    for policy_name, gateway in results.items():
        fog_nodes = gateway.fog_nodes
        power_values = [f'{node.power_consumption:.2f}' for node in fog_nodes]
        print(f"{policy_name}: {power_values}")
    ...
    for policy_name, gateway in results.items():
        ...
        print(f"{policy_name}: {avg_delay:.2f}")
    
    # Task distribution
    print("\n=== Task Distribution ===")
    for policy_name, gateway in results.items():
        # Count tasks by processor type using data_type_counts
        ...
        print(f"{policy_name}: Fog = {fog_count} ({fog_count/total*100:.1f}%), Cloud = {cloud_count} ({cloud_count/total*100:.1f}%)")
    ...
    for policy_name, gateway in results.items():
        ...
        # Use fixed value for cloud selection time if not available
        cloud_selection_times = gateway.metrics.get('cloud_selection_time', [1.0])
        avg_cloud_selection = sum(cloud_selection_times) / len(cloud_selection_times) if cloud_selection_times else 1.0
        
        print(f"{policy_name}: Node = {avg_selection:.4f}, Alt Node = {avg_alt_selection:.4f}, Cloud = {avg_cloud_selection:.4f}")
...
def main():
    ...
    # Automatically run all policies
    print("\nRunning all policies...")
    choice = 5
    
    # Map policy names to modules
    policy_modules = {
        "FCFSCooperation": FCFSC,
        "FCFSNoCooperation": FCFSN,
        "RandomCooperation": RANDOMC,
        "RandomNoCooperation": RANDOMN
    }
    
    # Run all policies
    results = {}
    for policy_name, module in policy_modules.items():
        results[policy_name] = run_policy(module, policy_name, tasks)
